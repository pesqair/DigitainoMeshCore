...
Dedicated view of all tracked repeater signal entries, sorted by most recently heard (youngest at top). Each row shows the repeater hex ID, RX signal bars (▼) and TX signal bars (▲) using the same visual style as the status bar, packet counts (rx/tx, matching bar order), and age. Columns are fixed-width so entries stay aligned regardless of missing data. TX column shows green bars when ping succeeded, red `X` when failed, or flashing `?` when a ping is in flight (steady `?` when idle/unknown).

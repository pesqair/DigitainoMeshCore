...
#define FIRMWARE_BUILD_DATE __DATE__   // compiler build date, e.g. "Jul  3 2026"
#endif
...
protected:
  ...
  void clearPendingReqs() {
    pending_login = pending_status = pending_telemetry = pending_discovery = pending_req = ui_pending_telemetry = ui_pending_status = ui_pending_ping_tag = ui_discover_tag = 0;
  }
...
private:
  ...
  uint32_t ui_pending_ping_tag;   // tag for UI-initiated ping (trace) request
  unsigned long ui_pending_ping_start; // millis() when ping was sent
  ...
  uint8_t _last_sent_hash[MAX_HASH_SIZE];
